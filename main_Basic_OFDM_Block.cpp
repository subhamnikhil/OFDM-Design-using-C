...
void compareErrorIPandOutput(float *data,float *dataout){
float error=0;
	for (int i = 0; i < FFTSize; i++) {
         error=error+abs((data[i]-dataout[i])/FFTSize);
...
void initializeZeroes(float *x_n_ifft,float *x_n_ifft_img,float *x_k_fft,float *x_k_fft_img){
...
// ML decoder (Minimum distance approach)
...
{
m[i]=i+1;  //[1 2]
M_sym_set[i]=2*m[i] - 1 - M; //=[-1.00 1.00]
}
...
for(int i=0;i<FFTSize;i++)
{
	for(int k=0;k<M;k++)
	{
    t[k] = (x_k_fft[i]-M_sym_set[k])*(x_k_fft[i]-M_sym_set[k]); //Distance calculation  (x-y)^2
	}
// Min distance
		if(t[0]<t[M-1])
		{
...
int  main()
{
	...
	float *x_k_bpsk,*x_k_fft,*rec_bpsk,*rec_bpsk_img;
...
printf("\n");
printf("\n");
initializeZeroes(x_n_ifft,x_n_ifft_img,x_k_fft,x_k_fft_img);
...
printf("\n--------- Rx side Removing cyclic prefix from Tx signal -------------------\n");
CyclicPrefixRxGenerator(x_n_cyclic,x_n_cyclic_img,x_k_cyclic,x_k_cyclic_img);
printf("\n");
...
compareErrorIPandOutput(data,dataout);
printf("\n");
